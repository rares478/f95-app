...
impl<R: Read> Deserializer<R> {
        ...
        loop {
            match self.read_byte()? {
                ...
                NEWOBJ => {
                    ...
                    let args = self.pop()?;
                    let class = self.pop_resolve()?;
                    // Ren'Py RevertableList/Set (and builtins.list/set) are built with
                    // NEWOBJ then APPEND/ADDITEMS. Recovering them as empty Dict breaks
                    // those opcodes — recover as the matching empty container instead.
                    match class {
                        Value::Global(Global::List) => {
                            let _ = args;
                            self.stack.push(Value::List(Vec::new()));
                        }
                        Value::Global(Global::Set) | Value::Global(Global::Frozenset) => {
                            let _ = args;
    ...
    fn decode_global(&mut self, modname: Vec<u8>, globname: Vec<u8>) -> Result<Value> {
        let value = match (&*modname, &*globname) {
            ...
            | (b"renpy.revertable", b"RevertableList") => Value::Global(Global::List),
