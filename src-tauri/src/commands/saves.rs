...
pub async fn wolf_saves_probe(install_path: String) -> Result<WolfProbeResult, AppError> {
    ...
    tokio::task::spawn_blocking(move || probe_wolf_install(&install))
        .await
        .map_err(|e| AppError::Other(format!("wolf_saves_probe join: {e}")))
}

#[tauri::command]
pub async fn wolf_saves_list(
    install_path: String,
    extra_roots: Option<Vec<ExtraSaveRoot>>,
) -> Result<Vec<RenpySaveSlot>, AppError> {
    let install = PathBuf::from(install_path);
    let roots = extra_roots_or_empty(extra_roots);
    tokio::task::spawn_blocking(move || wolf_list_for_install(&install, &roots))
        .await
        .map_err(|e| AppError::Other(format!("wolf_saves_list join: {e}")))?
}

#[tauri::command]
pub async fn wolf_save_read(
    install_path: String,
    slot_key: String,
    extra_roots: Option<Vec<ExtraSaveRoot>>,
) -> Result<RenpyVarNode, AppError> {
    let install = PathBuf::from(install_path);
    let roots = extra_roots_or_empty(extra_roots);
    tokio::task::spawn_blocking(move || wolf_read(&install, &slot_key, &roots))
        .await
        .map_err(|e| AppError::Other(format!("wolf_save_read join: {e}")))?
}

#[tauri::command]
pub async fn wolf_save_write(
    app: AppHandle,
    thread_id: String,
    install_path: String,
    slot_key: String,
    ...
    .await
    .map_err(|e| AppError::Other(format!("wolf_save_write join: {e}")))?
}

#[tauri::command]
pub async fn wolf_save_backups_list(
    app: AppHandle,
    thread_id: String,
    slot_key: String,
) -> Result<Vec<RenpySaveBackup>, AppError> {
    ...
    tokio::task::spawn_blocking(move || list_backups(&backups, &thread_id, &slot_key))
        .await
        .map_err(|e| AppError::Other(format!("wolf_save_backups_list join: {e}")))?
}

#[tauri::command]
pub async fn wolf_save_backup_restore(
    app: AppHandle,
    thread_id: String,
    install_path: String,
    slot_key: String,
