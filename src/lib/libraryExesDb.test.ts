...
describe('library multi-exe façade', () => {
  ...
  it('updateExePaths rejects path colliding with a sibling (DUPLICATE_EXE_PATH)', async () => {
    const a: LibraryGameExe = {
      id: 'a',
      threadId: 't1',
      exePath: 'D:/s1/game.exe',
    ...
    // select by id → listExes siblings — must not UPDATE after duplicate check
    query.mockResolvedValueOnce([toDbRow(b)]).mockResolvedValueOnce([toDbRow(a), toDbRow(b)]);

    await expect(
      library.updateExePaths('b', 'D:/s1/game.exe', 'D:/s1'),
    ).rejects.toThrow('DUPLICATE_EXE_PATH');
